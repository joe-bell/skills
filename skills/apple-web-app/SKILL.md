---
name: apple-web-app
description: Use when implementing, debugging, or auditing installed Apple web app UI on iPhone, iPad, or macOS Safari Add to Dock, including safe areas, icons, splash screens, and installed navigation. Not for service workers, offline caching, or push.
metadata:
...
Use this skill for installed iOS/iPadOS home-screen apps and macOS Safari
Add to Dock UI. Service workers, offline caching, background sync and push
...
authentication and product choices unless they cause the requested issue.
...
- **Audit:** use the [conformance checklist](references/conformance-checklist.md)
  for all applicable recommendations. Report findings before modifying the site
  unless the user has also asked for fixes.
...
- Preserve pinch zoom and text selection. Scope touch suppression to controls
  or media with an intentional replacement interaction. Source: Firtman;
  React Spectrum, in [sources.md](references/sources.md).
- Installed apps that hide browser controls need usable navigation back or
  out. Source: Firtman; Steiner, in [install-ux.md](references/install-ux.md).

Treat documented behavior, device observations, community workarounds and
optional design preferences distinctly. Apply a version-specific workaround
only when its symptom and environment match. A reported fix or bug status is
...
Source provenance and retained observations: [sources.md](references/sources.md).

## Verify the requested outcome

Use relevant entries from the [conformance checklist](references/conformance-checklist.md).
Check generated HTML, CSS, image dimensions and unauthenticated asset delivery
where applicable. Device-specific appearance needs observation on the target
installed app; desktop emulation alone does not establish it. Source: Joe Bell,
in [sources.md](references/sources.md).
